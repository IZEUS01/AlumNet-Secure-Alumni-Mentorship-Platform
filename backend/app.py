...
async function handleRegister(e) {
  ...
  // ✅ Success — go to login
  showAlert(alertEl, '✅ Account created! Please wait for admin approval before logging in.', 'success');
...
async function requireAuth(allowedRoles = []) {
  try {
    ...
    // Block access if account is not yet approved
    ...
    // Admin can access any page — skip role check
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loginAlert) {
    ...
    if (p.get('registered')) showAlert(loginAlert, '✅ Account created! An admin will review and approve your account before you can log in.', 'success');
    if (p.get('verified'))   showAlert(loginAlert, '✅ Email verified! You can now sign in.',  'success');
    if (p.get('status') === 'pending')  showAlert(loginAlert, '⏳ Your account is awaiting admin approval. Please check back later.');
    if (p.get('status') === 'rejected') showAlert(loginAlert, '❌ Your account registration was rejected. Please contact support.');
  }
});
